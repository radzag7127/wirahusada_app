...
class ThesisListPage extends StatefulWidget {
  ...
  @override
  State<ThesisListPage> createState() => _ThesisListPageState();
}

class _ThesisListPageState extends State<ThesisListPage> {
            ...
            child: BlocBuilder<LibraryBloc, LibraryState>(
              builder: (context, state) {
                if (state is LibraryLoading) {
                  ...
                  final allSkripsi = state.collections;
                  final filtered = allSkripsi.where((s) {
                    final title = s.judul.toLowerCase();
                    final author = s.penulis.toLowerCase();
                  ...
                  if (filtered.isEmpty) {
                    return const Center(
                        child: Text("Tidak ada skripsi ditemukan"));
                  }

                  return ListView.builder(
                    itemCount: filtered.length,
                    itemBuilder: (context, index) {
                      final skripsi = filtered[index];
                      return Padding(
                        ...
                        child: Container(
                          ...
                          child: ListTile(
                            ...
                            subtitle: Column(
                              ...
                              children: [
                                Text(skripsi.penulis,
                                    style: const TextStyle(fontSize: 12)),
                                Text(
                                  "${skripsi.penerbit ?? '-'} (${skripsi.tahun ?? '-'})".trim(),
                                  style: const TextStyle(
                                    fontSize: 12,
                            ...
                            trailing: IconButton(
                              onPressed: () => _openPDF(skripsi.filePdf),
                ...
                return const Center(child: Text("Memuat skripsi..."));
              },
            ),
          ),
        ],
